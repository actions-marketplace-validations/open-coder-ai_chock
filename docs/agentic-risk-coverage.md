...
| …a workflow pulling a third-party Action by a tag that can be re-pointed? | `pin-github-actions` gate | `enforced-at-commit`; `best-effort` at the write path on Claude Code and Gemini CLI, bounded to `.github/workflows/*` and `.github/actions/*` |
