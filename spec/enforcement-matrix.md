...
| ID | Spec | Check | Severity | Notes | Dispatch |
|---|---|---|---|---|---|
| POL-1 | `spec/policy-spec.md` §17 | `the validator`: `check_policy_toggles()` errors if `policies.disabled` contains a policy with `mandatory: true` | error | CLI `disable` also blocks this with exit code 2, and `policy_status()` ignores the entry so `sync` compiles the policy anyway | engine |
...
| POL-4 | `spec/policy-spec.md` §17 | `chock check --only baseline --base <ref>`: `check_baseline()` (`checks_baseline.py`) reads `.chock/config.yaml` at the base ref and at the head and errors on every policy the head disables, downgrades to advisory or narrows to fewer surfaces; wired into `chock check` by `lifecycle.py`'s `baseline` sub-check and run in CI against the pull request's base branch | error | Needs a base ref, so bare `chock check` skips it with a note and CI passes `origin/$GITHUB_BASE_REF`. POL-1 refuses disabling a *mandatory* policy anywhere; this refuses *weakening* any policy in a PR without a human seeing the diff. Widening is never a finding | lifecycle |
